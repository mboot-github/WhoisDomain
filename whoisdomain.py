...
def testDomains(aList):
    for d in aList:
        ...
        try:
            testItem(d)
        except whoisdomain.UnknownTld as e:
            errorItem(d, e, what="UnknownTld")
        except whoisdomain.FailedParsingWhoisOutput as e:
            errorItem(d, e, what="FailedParsingWhoisOutput")
        except whoisdomain.UnknownDateFormat as e:
            errorItem(d, e, what="UnknownDateFormat")
        except whoisdomain.WhoisCommandFailed as e:
            errorItem(d, e, what="WhoisCommandFailed")
        except whoisdomain.WhoisQuotaExceeded as e:
            errorItem(d, e, what="WhoisQuotaExceeded")
        except whoisdomain.WhoisPrivateRegistry as e:
            errorItem(d, e, what="WhoisPrivateRegistry")
        except whoisdomain.WhoisCommandTimeout as e:
...
test.py
    [ -v | --verbose ]
        # set verbose to True, this will be forwarded to whoisdomain.query

    [ -I | --IgnoreReturncode ]
        # sets the IgnoreReturncode to True, this will be forwarded to whoisdomain.query
    ...
    [ -C <file> | --Cleanup <file> ]
    read the input file specified and run the same cleanup as in whoisdomain.query , then exit
...
def main():
    ...
    for opt, arg in opts:
        if opt in ("-S", "SupportedTld"):
            for tld in sorted(whoisdomain.validTlds()):
                print(tld)
            sys.exit(0)
