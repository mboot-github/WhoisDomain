import sys
import time
from typing import (
    Any,
)

import dns.resolver
import requests_cache
from bs4 import BeautifulSoup


class IanaCrawler:
    URL: str = "https://www.iana.org/domains/root/db"

    CacheTime: int = 3600 * 24  # default 24 hours
    Session: Any = None
    cacheName: str = ".iana_cache"
    verbose: bool = False
    cacheBackend: str = "filesystem"

    records: list[Any] = []
    columns: list[Any] = []
    toDelete: list[str] = []

    def __init__(
        self,
        verbose: bool = False,
        resolver: Any = None,
    ):
        self.verbose = verbose
        self.resolver = resolver
        self.Session = requests_cache.CachedSession(
            self.cacheName,
            backend=self.cacheBackend,
        )

    def _getUrl(self) -> str:
        return self.URL

    """
    get the data from the url , with retry N times
    return the data as soup
    """

    def _getPageFromUrlIntoSoupWithRetry(
        self,
        url: str,
    ) -> BeautifulSoup:
        maxTry: int = 5
        n: int = 0
        response = None
        while n < maxTry:
            n += 1
            try:
                response = self.Session.get(url)
            except Exception as e:
                print(e, file=sys.stderr)
                time.sleep(5)

        return BeautifulSoup(response.text, "html.parser")

    def _getAdditionalItem(
        self,
        what: str,
        data: list[str],
    ) -> str | None:
        for i in [0, 1]:
            try:
                z: str = f"{what}:"
                if z in data[i]:
                    return data[i].replace(z, "").strip()
            except Exception as e:
                _ = e
                return None
        return None

    def _getTldParagraphWithString(
        self,
        soup: BeautifulSoup,
        text: str,
    ) -> str | None:
        gfg: list[Any] = soup.find_all(lambda tag: tag.name == "p" and text in tag.text)
        return gfg[0].text.strip() if len(gfg) else None

    def _resolveWhois(
        self,
        whois: str,
    ) -> list[Any]:
        ll: list[Any] = []

        if not self.resolver:
            print(f"no resolver set: {whois}")
            return None

        answer: list[Any] = []

        n: int = 3
        while n:
            try:
                x = dns.resolver.resolve(whois, "A")
                answer = list(x.response.answer)
                break
            except Exception as e:
                print(whois, e, n, file=sys.stderr)
                time.sleep(5)
                n -= 1

        for a in answer:
            s = str(a)
            if "\n" in s:
                ss = s.split("\n")
                ll.append(ss)
            else:
                ll.append(s)

        return ll

    def extractInfoFromPageSoup(
        self,
        page: BeautifulSoup,
        tldItem: list[Any],
    ) -> None:
        # URL for registration services: http://www.aaa.com
        # WHOIS Server: whois.nic.aaa
        # RDAP Server: https://rdap.nic.aaa/
        zz = {
            "Whois": "WHOIS Server",
            "RegistrationUrl": "URL for registration services",
            "Rdap": "RDAP Server",
        }
        for key, val in zz.items():
            regDataW: str | None = self._getTldParagraphWithString(page, val)
            if not regDataW:
                tldItem.append(None)
                continue

            regDataW = regDataW.replace(val, key)
            regDataA = regDataW.split("\n")
            for s in [key]:
                s = self._getAdditionalItem(s, regDataA)
                tldItem.append(s)

    def doWhoisServerResolve_DoesItExist(
        self,
        server: str | None,
        tldItem: list[Any],
    ) -> None:
        if server is None:
            tldItem.append(None)
            return

        ll = self._resolveWhois(server)
        print(ll)
        tldItem.append(ll)

    def _addInfoToOneTld(
        self,
        tld_item: list[Any],
    ) -> list[str]:
        this_tld_name = tld_item[0]

        if tld_item[3] == "Not assigned":
            tld_item[3] = None
            return None

        this_tld_url: str = self._getUrl() + "/" + this_tld_name + ".html"
        page = self._getPageFromUrlIntoSoupWithRetry(this_tld_url)

        self.extractInfoFromPageSoup(page, tld_item)
        print(tld_item[4])
        self.doWhoisServerResolve_DoesItExist(tld_item[4], tld_item)

        return tld_item

    def _processOneTableData(self, trs: list[str]) -> list[str]:
        record: list[str] = []
        for each in trs:
            try:
                link = each.find("a")["href"]
                aa = link.split("/")
                s = aa[-1].replace(".html", "")
                record.append(s)
                s = each.text.strip()
                record.append(s)
            except Exception as e:
                _ = f"{e}"
                record.append(each.text)
        return record

    def _processOneTableRow(self, tr: str) -> None:
        ths = tr.findAll("th")  # Table Header
        if ths != []:
            for each in ths:
                self.columns.append(each.text)
            return

        trs = tr.findAll("td")  # Table Data
        record = self._processOneTableData(trs)
        self.records.append(record)

    # Public
    def fromIanaGetAllCurrenlyDefinedTld(self) -> None:
        # from the main page we get all the current tld names
        page = self._getPageFromUrlIntoSoupWithRetry(self._getUrl())
        table: Any = page.find("table")  # from the first table we get the current defined tld records
        self.records: list[Any] = []
        self.columns: list[Any] = []

        for tr in table.findAll("tr"):  # Table Row
            self._processOneTableRow(tr)

        self.columns.insert(0, "Link")

    def addInfoToAllTld(self) -> None:
        records2: list[str] = []
        toDelete: list[str] = []

        self.columns[3] = self.columns[3].replace(" ", "_")
        self.columns.insert(4, "Whois")  # is there a whois server defined
        self.columns.insert(5, "RegistrationUrl")  # is there a registration url defined
        self.columns.insert(6, "Rdap")  # is there a whois server defined
        self.columns.insert(7, "DnsResolve-A")  # if we have a whois server does it actually resolve to sometething real

        for tldItem in self.records:  # tldItem is a list
            rr = self._addInfoToOneTld(tldItem)
            if rr is None:  # tld is not assigned any more
                toDelete.append(tldItem[0])
                continue
            records2.append(rr)

        self.records = records2
        self.toDelete = toDelete

    def getDeleted(
        self,
    ) -> list[str]:
        return self.toDelete

    def getResults(self) -> dict[str, Any]:
        ll = list(self.columns)
        return {
            "header": ll,
            "data": self.records,
        }
